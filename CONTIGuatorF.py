...
__credits__ = ["Felipe Luiz Pereira", "Lee Katz", "Florent Lassalle","Margaret Priest",
               "Luisa Santopolo","Francesca Decorosi","Mitchell Stanton-Cook"]
__license__ = "GPL"
__version__ = "2.7.3"
__maintainer__ = "Marco Galardini, Felipe Luiz Pereira"
...
def getOptions():
    ...
    group1.add_option('-g', '--genbank-annot', action="append", dest='lGenBankFiles', default=[],
                help='Arquivo com anotacao para apresentacao na fita da referencia')
    group1.add_option('-W', '--genbank-annot-barrnap', action="append", dest='lGenBankFilesBar', default=[],
                help='Arquivo com anotacao feita com barnnap')
...
def ManualACT(name, sRef, sPC, sCrunch, outdir, mylog, options):
    #felipe: adicionei options como parametro
    '''
    ...
    for f in genomes:
        ...
        tracks[f] = gd_diagram.new_track(1, name=f, start=0, end=len(records[f]),
                                         scale_smalltick_interval=50000,
                                         scale_largetick_interval=500000,
                                         scale_fontsize=10)
        ...
        for feat in record.features:
            if feat.type == "Contig":
                ...
                feature_set.add_feature(feat,
                                    ...
                                    border=colors.green)
    #felipestart
    #if os.path.isfile('ref_anot.gb'):
    titulo=""
    if options.lGenBankFiles != []:
        if os.path.isfile(options.lGenBankFiles[0]):
            rrnaebml = SeqIO.read(options.lGenBankFiles[0], 'gb')
            titulo=rrnaebml.description
            ftrack_set = feature_sets[sRef];
            for feat in rrnaebml.features:
        	indextogetinfo = "product"
                if feat.type == "rRNA":
                    ftrack_set.add_feature(feat, color=colors.blue,
                                    border=colors.blue,label=True,
                                    label_size=4, label_angle=45,
                                    label_position="middle",
                                    name = feat.qualifiers[indextogetinfo][0])
                if feat.type == "CDS":
                    cPhage = feat.qualifiers[indextogetinfo][0].count("hage")
                    if cPhage > 0:
                        ftrack_set.add_feature(feat, color=colors.lawngreen,
                                    border=colors.lawngreen,label=False,
                                    label_size=4, label_angle=45,
                                    label_position="middle",
                                    name = feat.qualifiers[indextogetinfo][0])
                    cPlasmid = feat.qualifiers[indextogetinfo][0].count("lasmid")
                    if cPlasmid > 0:
                        ftrack_set.add_feature(feat, color=colors.yellow,
                                    border=colors.yellow,label=True,
                                    label_size=4, label_angle=45,
                                    label_position="middle",
                                    name = feat.qualifiers[indextogetinfo][0])
                    cTranspo = feat.qualifiers[indextogetinfo][0].count("ransposase")
                    if cTranspo > 0:
                        ftrack_set.add_feature(feat, color=colors.lightblue,
                                    border=colors.lightblue,label=True,
                                    label_size=4, label_angle=45,
                                    label_position="middle",
                                    name = feat.qualifiers[indextogetinfo][0])
                indextogetinfo = "note"
            ...
            print "GenBank para anotacao nao encontrado..."
    if options.lGenBankFilesBar != []:
        if os.path.isfile(options.lGenBankFilesBar[0]):
            rrnaebml = SeqIO.read(options.lGenBankFilesBar[0], 'embl')
            titulo=rrnaebml.description
            ftrack_set = feature_sets[sRef];
            for feat in rrnaebml.features:
                if feat.type == "Hit":
            ...
            print "GenBank para anotacao criado pelo barnnap nao encontrado..."
    
    if titulo != "":
	#print gd_diagram
	gdt_features = gd_diagram.new_track(1, greytrack=False, scale_ticks=False)
	gds_features = gdt_features.new_set()
	
	feature = SeqFeature(FeatureLocation(25, 125), strand=+1)
	gds_features.add_feature(feature, name=titulo, label=True, label_size=12, label_angle=0)
	feature = SeqFeature(FeatureLocation(25, 125), strand=-1)
	gds_features.add_feature(feature, name=strftime("%d %B %Y - %H:%M:%S"), label=True, label_size=12, label_angle=180)
    ...
    if bPDF:
        for sRef in oCFs.references.keys():
            ...
            ManualACT(sRef.replace('.reference.fasta',''), oCFs.refembl[sRef],
                      oCFs.embl[sRef],
                      oCFs.crunch[sRef], sRefDir, mylog, options)
            #felipe adicinei options
